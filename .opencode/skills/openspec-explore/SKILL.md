...
- Use `dewey_semantic_search` to find conceptually related content across all indexed sources (specs, issues, docs)
- Use `dewey_similar` to find documents similar to the one being explored
- Use `dewey_traverse` to follow relationships between related documents
- Use `dewey_semantic_search_filtered` to narrow searches by source type (e.g., only GitHub issues, only web docs)
