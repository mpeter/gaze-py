...
- [ ] 1.1 Write `pyproject.toml` — `name="gaze-py"`, hatchling build backend, `requires-python = ">=3.11"`, `dependencies = ["click>=8.0", "pyyaml>=6.0"]` (no `rich` — CS-009 exception documented in design.md), binary `gazepy = "gaze.cli.main:cli"`, ruff/mypy/pytest config including `norecursedirs = ["tests/testdata"]` and `pythonpath = ["src"]`; `[tool.mypy]` with `strict = true`
...
- [ ] 1.6 Run `uv lock` (or confirm `uv.lock` was generated by `uv sync`) then `git add uv.lock` — MUST commit `uv.lock` to version control. CI uses `uv sync --frozen` which will fail without a committed lock file. This satisfies Constitution Principle VII (Supply Chain Integrity). Also add a SHA verification step: for each action in `.github/workflows/test.yml`, verify `gh api repos/<owner>/<repo>/git/ref/tags/<version>` resolves to the pinned SHA.
...
- [ ] 4.1 Write testdata fixtures (all in `tests/testdata/analysis/`):
  ...
  - `closure_capture_mutation.py` — `nonlocal x; x = new_val` inside a nested function (ClosureCaptureMutation, P4)
...
- [ ] 4.5 Run `uv run pytest tests/test_detector.py` — MUST pass; `uv run mypy src/gaze/analysis/` — MUST pass
...
- [ ] 8.1 Write `tests/test_cli.py` — tests FIRST (red), use `click.testing.CliRunner`:
  ...
  - `gazepy analyze <testdata_path> --coverage-json <coverage_file>` exits 0 AND JSON output contains non-null `line_coverage` (TC-008: assert output content, not just exit code); use `tests/testdata/coverage_sample.json` fixture (see task 8.2 note)
...
- [ ] 8.2 Write `src/gaze/cli/main.py` — `@click.group() cli`, `analyze` subcommand (path, `--format`, `--coverage-json`), `report` subcommand (src, tests, `--format`); wire detector → classifier → complexity → scorer → formatter pipeline; catch `GazeParseError` (imported from `taxonomy/exceptions.py`) and emit warning via `click.echo(err=True)`, continue; set `line_coverage=None` when `--coverage-json` not provided; when `--coverage-json` is invalid/malformed, raise `GazeConfigError` and exit non-zero; `report` command accepts `<tests>` argument but ignores it with a warning ("report --tests: O1 quality assessment deferred") — in this change, report behaves identically to analyze; add `tests/testdata/coverage_sample.json` as a minimal valid coverage.py JSON fixture (format: `{"files": {"<path>": {"summary": {"percent_covered": 75.0}}}}` — coverage.py v6+ schema, field `files[path].summary.percent_covered`)
