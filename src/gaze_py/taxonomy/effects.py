...
Defines the canonical 48-value SideEffectType enum, the 5-tier Tier enum,
...
be configurable. The contract tier table totals 48 (P0=6 + P1=11 + P2=16 +
P3=9 + P4=6). Ten of these are "Defined" in taxonomy-reference.md — specified
in the taxonomy but not yet detected by the reference Go implementation.
They are included here as definitions; detection is tracked separately.
"""
...
class SideEffectType(enum.StrEnum):
    ...
    # DEFINED, DETECTION PENDING (taxonomy-reference.md status: Defined).
    # Language-neutral error signaling (`raise`, `throw`, `return err`).
    # The reference Go implementation labels these ErrorReturn; gaze-py
    # mirrors that behavior. Emitting ErrorSignal awaits an upstream
    # contract decision on the ErrorReturn/ErrorSignal split.
    ...
    # DEFINED, DETECTION PENDING (taxonomy-reference.md status: Defined).
    # Not yet detected by the reference Go implementation either.
