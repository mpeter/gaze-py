...
- [ ] 1.1 Add flat `ai_*` fields to `GazeConfig` in `src/gaze_py/config/loader.py`: `ai_provider: str = ""`, `ai_model: str = ""`, `ai_endpoint: str = ""`, `ai_project: str = ""`, `ai_region: str = ""`, `ai_timeout: int = 120`
- [ ] 1.2 Parse `ai:` YAML block in `_build_config` using existing key-extraction pattern (consistent with `classification:` and `scoring:` blocks); silently ignore unknown keys
- [ ] 1.3 Add `ai_timeout` validation to `_validate`: must be `> 0`; error message format: `"ai.timeout must be > 0, got X in /path/.gaze.yaml"`; coerce float input to int via existing `_to_int` helper
- [ ] 1.4 Add tests for ai field parsing, defaults, timeout validation, float coercion, and unknown key handling to `tests/test_config.py`
...
- [ ] 5.1 Add `CHANGELOG.md` entry documenting removal of `--ai` and `--ai-timeout` with migration note: configure provider via `.gaze.yaml` `ai:` section or `GAZEPY_AI_*` env vars
- [ ] 5.2 Update `README.md` with `## AI Reports` section: configuration via `.gaze.yaml` `ai:` block, all env vars with descriptions, Ollama prerequisites (model pulled), Vertex prerequisites (`gcloud` installed + `gcloud auth application-default login`), and troubleshooting
- [ ] 5.3 Update `pyproject.toml` project URLs to `github.com/unbound-force/gaze-py`

## 6. Gate check

- [ ] 6.1 `uv run ruff check .` passes
- [ ] 6.2 `uv run ruff format --check .` passes
- [ ] 6.3 `uv run mypy --strict src/` passes
- [ ] 6.4 `uv run pytest -m "not slow" --no-cov` passes (all tests green, no network access, no real sleeping)
- [ ] 6.5 `uv run pytest` passes (pyproject.toml addopts enforce `--cov=gaze_py --cov-fail-under=85`)
- [ ] 6.6 Verify `test.yml` requires no changes — no new secrets, services, or env vars needed in CI
