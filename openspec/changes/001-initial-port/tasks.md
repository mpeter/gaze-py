...
- [ ] 1.1 Write `pyproject.toml` — `name="gaze-py"`, hatchling build backend, `requires-python = ">=3.11"`, `dependencies = ["click>=8.0,<9", "pyyaml>=6.0,<7"]` (no `rich` — CS-009 exception documented in design.md), binary `gazepy = "gaze_py.cli.main:cli"`, ruff/mypy/pytest config including `norecursedirs = ["tests/testdata"]` and `pythonpath = ["src"]`; `[tool.mypy]` with `strict = true`
- [ ] 1.2 [P] Write `src/gaze_py/__init__.py` — `__version__ = "0.1.0"` only
- [ ] 1.3 [P] Write all `__init__.py` files with module docstrings: `src/gaze_py/taxonomy/`, `src/gaze_py/analysis/`, `src/gaze_py/classify/`, `src/gaze_py/classify/signals/`, `src/gaze_py/crap/`, `src/gaze_py/config/`, `src/gaze_py/report/`, `src/gaze_py/cli/`
