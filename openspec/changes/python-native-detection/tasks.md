...
## 1. New constants in detector.py
...
## 3. async with param → MutexOp / DatabaseTransaction

- [ ] 3.1 In `visit_AsyncWith`, add param-based lock detection immediately
      **before** the existing TaskGroup detection loop. The full method becomes:
      ...
      def visit_AsyncWith(self, node: ast.AsyncWith) -> None:  # noqa: N802
          ...
          Param-based patterns (async with lock:, async with conn:):
              Same name heuristics as visit_With — connection/session/tx names
              → DatabaseTransaction; all others → MutexOp.
