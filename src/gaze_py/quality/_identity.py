...
class _ImportBinding:
    """One statically known local import binding."""
...
class _TargetIdentity:
    ...
    def matches(self, call: ast.Call, bindings: dict[str, _ImportBinding]) -> bool:
        ...
        if isinstance(call.func, ast.Name):
            ...
            return binding == _ImportBinding(self.module, self.function)
        if not isinstance(call.func, ast.Attribute) or call.func.attr != self.function:
            return False
        if not isinstance(call.func.value, ast.Name):
            return False
        return bindings.get(call.func.value.id) == _ImportBinding(self.module)
...
def _collect_module_aliases(module: ast.Module) -> tuple[ModuleAlias, ...]:
    """Collect ordered module-level imports that remain unshadowed."""
    aliases: dict[str, ModuleAlias] = {}
    for statement in module.body:
        if isinstance(statement, ast.Import):
            for imported in statement.names:
                ...
                aliases[local_name] = ModuleAlias(local_name, bound_module)
            ...
            for imported in statement.names:
                ...
                if statement.level == 0 and statement.module is not None:
                    aliases[local_name] = ModuleAlias(
                        local_name,
                        statement.module,
                        imported.name,
                    )
