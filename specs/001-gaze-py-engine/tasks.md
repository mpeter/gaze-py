...
- [x] T022 [S5] Write tests for new setup step in
...
- [x] T023 [S5] Add `installGazePy()` to
...
- [x] T024 [S5] Add `gaze-py` to the setup step list in
...
- [x] T025 [P] [S5] Create scaffold asset
...
- [x] T026 [P] [S5] Run `unbound-force` CI parity:
  `go test ./...`
  `go vet ./...`
  Fix all issues before marking complete.

- [x] T027 [S5] Update documentation (after T026 green):
...
- [x] `go test ./...` — all green (unbound-force repo)
- [x] `uf init --dry-run` on Python project shows gaze-py step with pinned version
- [x] AGENTS.md updated in both repos
- [x] CHANGELOG.md entries in both repos
- [x] `unbound-force/website` issue filed for new CLI commands (`analyze`, `quality`, `report`) and `uf init` gaze-py step
